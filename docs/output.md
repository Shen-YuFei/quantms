...
### Comet binning provenance

For each Comet run, `nextflow.log` records the original fragment tolerance, the native full bin width in Da, the half width passed to OpenMS, the offset and instrument mode, and the source of those settings. Per-task `.command.sh` records the actual adapter arguments. These Comet-specific settings do not replace the SDRF tolerance supplied to other engines or rescoring.
